...
/* `#END` */
    ...
    *******************************************************************************/
    void CAN_ArbLostIsr(void) 
    {
        ...
        CAN_INT_SR_REG = CAN_ARBITRATION_LOST_MASK;
    ...
    *******************************************************************************/
    void CAN_OvrLdErrorIsr(void) 
    {
        ...
        CAN_INT_SR_REG = CAN_OVERLOAD_ERROR_MASK;
    ...
    *******************************************************************************/
    void CAN_BitErrorIsr(void) 
    {
        ...
        CAN_INT_SR_REG = CAN_BIT_ERROR_MASK;
    ...
    *******************************************************************************/
    void CAN_BitStuffErrorIsr(void) 
    {
        ...
        CAN_INT_SR_REG = CAN_STUFF_ERROR_MASK;
    ...
    *******************************************************************************/
    void CAN_AckErrorIsr(void) 
    {
        ...
        CAN_INT_SR_REG = CAN_ACK_ERROR_MASK;
    ...
    *******************************************************************************/
    void CAN_MsgErrorIsr(void) 
    {
        ...
        CAN_INT_SR_REG = CAN_FORM_ERROR_MASK;
    ...
    *******************************************************************************/
    void CAN_CrcErrorIsr(void) 
    {
        ...
        CAN_INT_SR_REG = CAN_CRC_ERROR_MASK;
    ...
    *******************************************************************************/
    ...
    {
        /* Clear Bus Off flag */
        CAN_INT_SR_REG = CAN_BUS_OFF_MASK;
    ...
    *******************************************************************************/
    void CAN_MsgLostIsr(void) 
    {
        ...
        CAN_INT_SR_REG = CAN_RX_MSG_LOST_MASK;
    ...
    *******************************************************************************/
    void CAN_MsgTXIsr(void) 
    {
        ...
        CAN_INT_SR_REG = CAN_TX_MESSAGE_MASK;
    ...
    *******************************************************************************/
    void CAN_MsgRXIsr(void) 
    {
        ...
        for (mailboxNumber = 0u; mailboxNumber < CAN_NUMBER_OF_RX_MAILBOXES; mailboxNumber++)
        {
            ...
            if ((CAN_BUF_SR_REG & shift) != 0u)
            {
                if ((CAN_RX_CMD_REG(mailboxNumber) & CAN_RX_INT_ENABLE_MASK) != 0u)
                {
                    if ((CAN_RX_MAILBOX_TYPE & shift) != 0u)
                    {
                        ...
                        switch(mailboxNumber)
                        {
                            ...
                            break;
                            case 1u : CAN_ReceiveMsg1();
                            break;
                            case 2u : CAN_ReceiveMsg2();
                            break;
    ...
    /*******************************************************************************
    * FUNCTION NAME:   CAN_RtrAutoMsgSentIsr
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    void CAN_RtrAutoMsgSentIsr(void)
    {
        ...
        CAN_INT_SR_REG = CAN_RTR_MSG_MASK;

        /* `#START RTR_AUTO_MSG_SENT_ISR` */

        /* `#END` */

        #ifdef CAN_RTR_AUTO_MSG_SENT_ISR_CALLBACK
            CAN_RtrAutoMsgSentIsr_Callback();
        #endif /* CAN_RTR_AUTO_MSG_SENT_ISR_CALLBACK */
    }
#endif /* CAN_RTR_AUTO_MSG_SENT */


#if (CAN_STUCK_AT_ZERO)
    /*******************************************************************************
    * FUNCTION NAME:   CAN_StuckAtZeroIsr
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    void CAN_StuckAtZeroIsr(void)
    {
        ...
        CAN_INT_SR_REG = CAN_STUCK_AT_ZERO_MASK;

        /* `#START STUCK_AT_ZERO_ISR` */

        /* `#END` */

        #ifdef CAN_STUCK_AT_ZERO_ISR_CALLBACK
            CAN_StuckAtZeroIsr_Callback();
        #endif /* CAN_STUCK_AT_ZERO_ISR_CALLBACK */
    ...
    *******************************************************************************/
    void CAN_SSTErrorIsr(void)
    {
        ...
        CAN_INT_SR_REG = CAN_SST_FAILURE_MASK;

        /* `#START SST_FAILURE_ISR_PRE` */

        /* `#END` */

        #ifdef CAN_SST_ERROR_ISR_PRE_CALLBACK
            CAN_SSTErrorIsr_Pre_Callback();
        #endif /* CAN_SST_ERROR_ISR_PRE_CALLBACK */

        /* Removes message that failed SST transmission from the Tx mailbox */
        for (i = 0u; i < CAN_NUMBER_OF_TX_MAILBOXES; i++)
        {
            if ((CAN_TX_CMD_REG(i) & CAN_TX_ABORT_MASK) != 0u)
            {
                CAN_TX_CMD_REG(i) = (CAN_TX_CMD_REG(i) & (uint32) (~(uint32) CAN_TX_TRANSMIT_REQUEST)) |
                                    CAN_TX_ABORT_MASK;
            }
        }
        ...
        #ifdef CAN_SST_ERROR_ISR_POST_CALLBACK
            CAN_SSTErrorIsr_Post_Callback();
        #endif /* CAN_SST_ERROR_ISR_POST_CALLBACK */
    }
#endif /* CAN_SST_FAILURE */
...
CY_ISR(CAN_ISR)
{
    ...
    /* Arbitration */
        ...
        if ((CAN_INT_SR_REG & CAN_ARBITRATION_LOST_MASK) != 0u)
        {
            CAN_ArbLostIsr();
        }
    ...
    /* Overload Error */
        ...
        (!CAN_ADVANCED_INTERRUPT_CFG)))
        if ((CAN_INT_SR_REG & CAN_OVERLOAD_ERROR_MASK) != 0u)
        {
            CAN_OvrLdErrorIsr();
        }
    ...
    /* Bit Error */
        ...
        (!CAN_ADVANCED_INTERRUPT_CFG)))
        if ((CAN_INT_SR_REG & CAN_BIT_ERROR_MASK) != 0u)
        {
            CAN_BitErrorIsr();
        }
    ...
    /* Bit Staff Error */
        ...
        (!CAN_ADVANCED_INTERRUPT_CFG)))
        if ((CAN_INT_SR_REG & CAN_STUFF_ERROR_MASK) != 0u)
        {
    ...
    /* ACK Error */
        ...
        (!CAN_ADVANCED_INTERRUPT_CFG)))
        if ((CAN_INT_SR_REG & CAN_ACK_ERROR_MASK) != 0u)
        {
            CAN_AckErrorIsr();
        }
    ...
    /* Form(msg) Error */
        ...
        (!CAN_ADVANCED_INTERRUPT_CFG)))
        if ((CAN_INT_SR_REG & CAN_FORM_ERROR_MASK) != 0u)
        {
            CAN_MsgErrorIsr();
        }
    ...
    /* CRC Error */
        ...
        (!CAN_ADVANCED_INTERRUPT_CFG)))
        if ((CAN_INT_SR_REG & CAN_CRC_ERROR_MASK) != 0u)
        {
            CAN_CrcErrorIsr();
        }
    ...
    /* Bus Off state */
        ...
        (!CAN_ADVANCED_INTERRUPT_CFG)))
        if ((CAN_INT_SR_REG & CAN_BUS_OFF_MASK) != 0u)
        {
            CAN_BusOffIsr();
        }
    ...
    /* Message Lost */
        ...
        (!CAN_ADVANCED_INTERRUPT_CFG)))
        if ((CAN_INT_SR_REG & CAN_RX_MSG_LOST_MASK) != 0u)
        {
            CAN_MsgLostIsr();
        }
    ...
    /* TX Message Send */
        ...
        (!CAN_ADVANCED_INTERRUPT_CFG)))
        if ((CAN_INT_SR_REG & CAN_TX_MESSAGE_MASK) != 0u)
        {
            CAN_MsgTXIsr();
        }
    ...
    /* RX Message Available */
        ...
        (!CAN_ADVANCED_INTERRUPT_CFG)))
        if ((CAN_INT_SR_REG & CAN_RX_MESSAGE_MASK) != 0u)
        {
            CAN_MsgRXIsr();
        }
    ...
    #if (CAN_RTR_AUTO_MSG_SENT && (CAN_RTR_MESSAGE_USE_HELPER || \
        (!CAN_ADVANCED_INTERRUPT_CFG)))
        if ((CAN_INT_SR_REG & CAN_RTR_MSG_MASK) != 0u)
        {
            CAN_RtrAutoMsgSentIsr();
        }
    #endif /* CAN_RTR_AUTO_MSG_SENT && CAN_RTR_MESSAGE_USE_HELPER */

    /* Stuck at dominant error */
    #if (CAN_STUCK_AT_ZERO && (CAN_STUCK_AT_ZERO_USE_HELPER || \
        (!CAN_ADVANCED_INTERRUPT_CFG)))
        if ((CAN_INT_SR_REG & CAN_STUCK_AT_ZERO_MASK) != 0u)
        {
            CAN_StuckAtZeroIsr();
        }
    #endif /* CAN_STUCK_AT_ZERO && CAN_STUCK_AT_ZERO_USE_HELPER */

    /* Single shot transmission failure */
    #if (CAN_SST_FAILURE && (CAN_SST_FAILURE_USE_HELPER || \
        (!CAN_ADVANCED_INTERRUPT_CFG)))
        if ((CAN_INT_SR_REG & CAN_SST_FAILURE_MASK) != 0u)
        {
            CAN_SSTErrorIsr();
        }
    #endif /* CAN_SST_FAILURE && CAN_SST_FAILURE_USE_HELPER */
}
