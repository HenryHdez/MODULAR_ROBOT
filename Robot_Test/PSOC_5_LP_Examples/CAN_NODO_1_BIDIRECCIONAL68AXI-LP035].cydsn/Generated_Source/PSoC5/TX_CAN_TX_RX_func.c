...
extern uint8 dato_recibido;
/* `#END` */


/*******************************************************************************
* FUNCTION NAME:   TX_CAN_SendMsg
********************************************************************************
*
* Summary:
...
uint8 TX_CAN_SendMsg(const TX_CAN_TX_MSG *message) 
{
    ...
    while (retry < TX_CAN_RETRY_NUMBER)
    {
        shift = 1u;    /* Start from first mailbox */
        for (i = 0u; i < TX_CAN_NUMBER_OF_TX_MAILBOXES; i++)
        {
            /* Find Basic TX mailboxes */
            if ((TX_CAN_TX_MAILBOX_TYPE & shift) == 0u)
            {
                /* Find free mailbox */
                #if (CY_PSOC3 || CY_PSOC5)
                    if ((TX_CAN_BUF_SR_REG.byte[2] & shift) == 0u)
                #else  /* CY_PSOC4 */
                    if ((TX_CAN_BUF_SR_REG &
                        (uint32) ((uint32) shift << TX_CAN_TWO_BYTE_OFFSET)) == 0u)
                #endif /* CY_PSOC3 || CY_PSOC5 */
                    {
                        regTemp = 0u;

                        /* Set message parameters */
                        if (message->rtr != TX_CAN_STANDARD_MESSAGE)
                        {
                            regTemp = TX_CAN_TX_RTR_MASK;    /* RTR message Enable */
                        }

                        if (message->ide == TX_CAN_STANDARD_MESSAGE)
                        {
                            TX_CAN_SET_TX_ID_STANDARD_MSG(i, message->id);
                        }
                        else
                        {
                            regTemp |= TX_CAN_TX_IDE_MASK;
                            TX_CAN_SET_TX_ID_EXTENDED_MSG(i, message->id);
                        }

                        if (message->dlc < TX_CAN_TX_DLC_MAX_VALUE)
                        {
                            regTemp |= ((uint32) message->dlc) << TX_CAN_TWO_BYTE_OFFSET;
                        }
                        else
                        {
                            regTemp |= TX_CAN_TX_DLC_UPPER_VALUE;
                        }

                        if (message->irq != TX_CAN_TRANSMIT_INT_DISABLE)
                        {
                            regTemp |= TX_CAN_TX_INT_ENABLE_MASK;    /* Transmit Interrupt Enable */
                        }

                        for (j = 0u; (j < message->dlc) && (j < TX_CAN_TX_DLC_MAX_VALUE); j++)
                        {
                            #if (CY_PSOC3 || CY_PSOC5)
                                TX_CAN_TX_DATA_BYTE(i, j) = message->msg->byte[j];
                            #else /* CY_PSOC4 */
                                TX_CAN_TX_DATA_BYTE(i, j, message->msg->byte[j]);
                            #endif /* CY_PSOC3 || CY_PSOC5 */
                        }
    ...
    CyIntDisable(TX_CAN_ISR_NUMBER);
                        ...
                        CY_SET_REG32(TX_CAN_TX_CMD_PTR(i), (regTemp | TX_CAN_TX_WPN_SET));

                        #if (CY_PSOC3 || CY_PSOC5)
                            CY_SET_REG32(TX_CAN_TX_CMD_PTR(i), TX_CAN_SEND_MESSAGE);
                        #else /* CY_PSOC4 */
                            if (message->sst != TX_CAN_STANDARD_MESSAGE)
                            {
                                /* Single Shot Transmission */
                                TX_CAN_TX_CMD_REG(i) |= TX_CAN_SEND_MESSAGE |
                                TX_CAN_TX_ABORT_MASK;
                            }
                            else
                            {
                                TX_CAN_TX_CMD_REG(i) |= TX_CAN_SEND_MESSAGE;
...
{
    if (bufferId < TX_CAN_NUMBER_OF_TX_MAILBOXES)
    {
        TX_CAN_TX_ABORT_MESSAGE(bufferId);
    }
}
    ...
    *******************************************************************************/
    uint8 TX_CAN_SendMsg0(void) 
    {
        uint8 result = CYRET_SUCCESS;
        ...
        #if (CY_PSOC3 || CY_PSOC5)
            if ((TX_CAN_TX[0u].txcmd.byte[0u] & TX_CAN_TX_REQUEST_PENDING) != 0u)
        #else  /* CY_PSOC4 */
            if ((TX_CAN_TX_CMD_REG(0u) & TX_CAN_TX_REQUEST_PENDING) != 0u)
        #endif /* CY_PSOC3 || CY_PSOC5 */
            {
                result = TX_CAN_FAIL;
            }
            else
            {
                ...
                TX_CAN_TX_DATA_BYTE1(0)=dato_enviado;
                /* `#END` */

                #ifdef TX_CAN_SEND_MSG_0_CALLBACK
                    TX_CAN_SendMsg_0_Callback();
                #endif /* TX_CAN_SEND_MSG_0_CALLBACK */

                CY_SET_REG32(TX_CAN_TX_CMD_PTR(0u),
                CY_GET_REG32(TX_CAN_TX_CMD_PTR(0u)) | TX_CAN_SEND_MESSAGE);
            }
    ...
    *******************************************************************************/
    uint8 TX_CAN_SendMsg1(void) 
    {
        uint8 result = CYRET_SUCCESS;
        ...
        #if (CY_PSOC3 || CY_PSOC5)
            if ((TX_CAN_TX[1u].txcmd.byte[0u] & TX_CAN_TX_REQUEST_PENDING) != 0u)
        #else  /* CY_PSOC4 */
            if ((TX_CAN_TX_CMD_REG(1u) & TX_CAN_TX_REQUEST_PENDING) != 0u)
        #endif /* CY_PSOC3 || CY_PSOC5 */
            {
                result = TX_CAN_FAIL;
            }
            else
            {
                ...
                TX_CAN_TX_DATA_BYTE1(1)=dato_enviado+1;
                /* `#END` */

                #ifdef TX_CAN_SEND_MSG_1_CALLBACK
                    TX_CAN_SendMsg_1_Callback();
                #endif /* TX_CAN_SEND_MSG_1_CALLBACK */

                CY_SET_REG32(TX_CAN_TX_CMD_PTR(1u),
                CY_GET_REG32(TX_CAN_TX_CMD_PTR(1u)) | TX_CAN_SEND_MESSAGE);
            }
    ...
    *******************************************************************************/
    uint8 TX_CAN_SendMsg2(void) 
    {
        uint8 result = CYRET_SUCCESS;
        ...
        #if (CY_PSOC3 || CY_PSOC5)
            if ((TX_CAN_TX[2u].txcmd.byte[0u] & TX_CAN_TX_REQUEST_PENDING) != 0u)
        #else  /* CY_PSOC4 */
            if ((TX_CAN_TX_CMD_REG(2u) & TX_CAN_TX_REQUEST_PENDING) != 0u)
        #endif /* CY_PSOC3 || CY_PSOC5 */
            {
                result = TX_CAN_FAIL;
            }
            else
            {
                ...
                TX_CAN_TX_DATA_BYTE1(2)=dato_enviado+2;
                /* `#END` */

                #ifdef TX_CAN_SEND_MSG_2_CALLBACK
                    TX_CAN_SendMsg_2_Callback();
                #endif /* TX_CAN_SEND_MSG_2_CALLBACK */

                CY_SET_REG32(TX_CAN_TX_CMD_PTR(2u),
                CY_GET_REG32(TX_CAN_TX_CMD_PTR(2u)) | TX_CAN_SEND_MESSAGE);
            }
    ...
    *******************************************************************************/
    uint8 TX_CAN_SendMsg3(void) 
    {
        uint8 result = CYRET_SUCCESS;
        ...
        #if (CY_PSOC3 || CY_PSOC5)
            if ((TX_CAN_TX[3u].txcmd.byte[0u] & TX_CAN_TX_REQUEST_PENDING) != 0u)
        #else  /* CY_PSOC4 */
            if ((TX_CAN_TX_CMD_REG(3u) & TX_CAN_TX_REQUEST_PENDING) != 0u)
        #endif /* CY_PSOC3 || CY_PSOC5 */
            {
                result = TX_CAN_FAIL;
            }
            else
            {
                ...
                TX_CAN_TX_DATA_BYTE1(3)=dato_enviado+3;
                /* `#END` */

                #ifdef TX_CAN_SEND_MSG_3_CALLBACK
                    TX_CAN_SendMsg_3_Callback();
                #endif /* TX_CAN_SEND_MSG_3_CALLBACK */

                CY_SET_REG32(TX_CAN_TX_CMD_PTR(3u),
                CY_GET_REG32(TX_CAN_TX_CMD_PTR(3u)) | TX_CAN_SEND_MESSAGE);
            }
    ...
    *******************************************************************************/
    uint8 TX_CAN_SendMsg4(void) 
    {
        uint8 result = CYRET_SUCCESS;
        ...
        #if (CY_PSOC3 || CY_PSOC5)
            if ((TX_CAN_TX[4u].txcmd.byte[0u] & TX_CAN_TX_REQUEST_PENDING) != 0u)
        #else  /* CY_PSOC4 */
            if ((TX_CAN_TX_CMD_REG(4u) & TX_CAN_TX_REQUEST_PENDING) != 0u)
        #endif /* CY_PSOC3 || CY_PSOC5 */
            {
                result = TX_CAN_FAIL;
            }
            else
            {
                ...
                #ifdef TX_CAN_SEND_MSG_4_CALLBACK
                    TX_CAN_SendMsg_4_Callback();
                #endif /* TX_CAN_SEND_MSG_4_CALLBACK */

                CY_SET_REG32(TX_CAN_TX_CMD_PTR(4u),
                CY_GET_REG32(TX_CAN_TX_CMD_PTR(4u)) | TX_CAN_SEND_MESSAGE);
            }
    ...
    *******************************************************************************/
    uint8 TX_CAN_SendMsg5(void) 
    {
        uint8 result = CYRET_SUCCESS;
        ...
        #if (CY_PSOC3 || CY_PSOC5)
            if ((TX_CAN_TX[5u].txcmd.byte[0u] & TX_CAN_TX_REQUEST_PENDING) != 0u)
        #else  /* CY_PSOC4 */
            if ((TX_CAN_TX_CMD_REG(5u) & TX_CAN_TX_REQUEST_PENDING) != 0u)
        #endif /* CY_PSOC3 || CY_PSOC5 */
            {
                result = TX_CAN_FAIL;
            }
            else
            {
                ...
                #ifdef TX_CAN_SEND_MSG_5_CALLBACK
                    TX_CAN_SendMsg_5_Callback();
                #endif /* TX_CAN_SEND_MSG_5_CALLBACK */

                CY_SET_REG32(TX_CAN_TX_CMD_PTR(5u),
                CY_GET_REG32(TX_CAN_TX_CMD_PTR(5u)) | TX_CAN_SEND_MESSAGE);
            }
    ...
    *******************************************************************************/
    uint8 TX_CAN_SendMsg6(void) 
    {
        uint8 result = CYRET_SUCCESS;
        ...
        #if (CY_PSOC3 || CY_PSOC5)
            if ((TX_CAN_TX[6u].txcmd.byte[0u] & TX_CAN_TX_REQUEST_PENDING) != 0u)
        #else  /* CY_PSOC4 */
            if ((TX_CAN_TX_CMD_REG(6u) & TX_CAN_TX_REQUEST_PENDING) != 0u)
        #endif /* CY_PSOC3 || CY_PSOC5 */
            {
                result = TX_CAN_FAIL;
            }
            else
            {
                ...
                #ifdef TX_CAN_SEND_MSG_6_CALLBACK
                    TX_CAN_SendMsg_6_Callback();
                #endif /* TX_CAN_SEND_MSG_6_CALLBACK */

                CY_SET_REG32(TX_CAN_TX_CMD_PTR(6u),
                CY_GET_REG32(TX_CAN_TX_CMD_PTR(6u)) | TX_CAN_SEND_MESSAGE);
            }
    ...
    *******************************************************************************/
    uint8 TX_CAN_SendMsg7(void) 
    {
        uint8 result = CYRET_SUCCESS;
        ...
        #if (CY_PSOC3 || CY_PSOC5)
            if ((TX_CAN_TX[7u].txcmd.byte[0u] & TX_CAN_TX_REQUEST_PENDING) != 0u)
        #else  /* CY_PSOC4 */
            if ((TX_CAN_TX_CMD_REG(7u) & TX_CAN_TX_REQUEST_PENDING) != 0u)
        #endif /* CY_PSOC3 || CY_PSOC5 */
            {
                result = TX_CAN_FAIL;
            }
            else
            {
                ...
                #ifdef TX_CAN_SEND_MSG_7_CALLBACK
                    TX_CAN_SendMsg_7_Callback();
                #endif /* TX_CAN_SEND_MSG_7_CALLBACK */

                CY_SET_REG32(TX_CAN_TX_CMD_PTR(7u),
                CY_GET_REG32(TX_CAN_TX_CMD_PTR(7u)) | TX_CAN_SEND_MESSAGE);
            }
...
/*******************************************************************************
* FUNCTION NAME:   TX_CAN_ReceiveMsg
********************************************************************************
*
* Summary:
...
{
    #if (CY_PSOC3 || CY_PSOC5)
        if ((TX_CAN_RX[rxMailbox].rxcmd.byte[0u] & TX_CAN_RX_ACK_MSG) != 0u)
    #else  /* CY_PSOC4 */
        if ((TX_CAN_RX_CMD_REG(rxMailbox) & TX_CAN_RX_ACK_MSG) != 0u)
    #endif /* CY_PSOC3 || CY_PSOC5 */
        {
            ...
            #ifdef TX_CAN_RECEIVE_MSG_CALLBACK
                TX_CAN_ReceiveMsg_Callback();
            #endif /* TX_CAN_RECEIVE_MSG_CALLBACK */

            #if (CY_PSOC3 || CY_PSOC5)
                TX_CAN_RX[rxMailbox].rxcmd.byte[0u] |= TX_CAN_RX_ACK_MSG;
            #else  /* CY_PSOC4 */
                TX_CAN_RX_CMD_REG(rxMailbox) |= TX_CAN_RX_ACK_MSG;
            #endif /* CY_PSOC3 || CY_PSOC5 */
        }
}
    ...
    /*******************************************************************************
    * FUNCTION NAME:   TX_CAN_ReceiveMsg0
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    void TX_CAN_ReceiveMsg0(void) 
    {
        ...
        dato_recibido=TX_CAN_RX_DATA_BYTE1(0);
        /* `#END` */

        #ifdef TX_CAN_RECEIVE_MSG_0_CALLBACK
            TX_CAN_ReceiveMsg_0_Callback();
        #endif /* TX_CAN_RECEIVE_MSG_0_CALLBACK */

        TX_CAN_RX[0u].rxcmd.byte[0u] |= TX_CAN_RX_ACK_MSG;
    }
    ...
    /*******************************************************************************
    * FUNCTION NAME:    TX_CAN_ReceiveMsg1
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    void TX_CAN_ReceiveMsg1(void) 
    {
        ...
        #ifdef TX_CAN_RECEIVE_MSG_1_CALLBACK
            TX_CAN_ReceiveMsg_1_Callback();
        #endif /* TX_CAN_RECEIVE_MSG_1_CALLBACK */

        TX_CAN_RX[1u].rxcmd.byte[0u] |= TX_CAN_RX_ACK_MSG;
    }
    ...
    /*******************************************************************************
    * FUNCTION NAME:   TX_CAN_ReceiveMsg2
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    void TX_CAN_ReceiveMsg2(void) 
    {
        ...
        #ifdef TX_CAN_RECEIVE_MSG_2_CALLBACK
            TX_CAN_ReceiveMsg_2_Callback();
        #endif /* TX_CAN_RECEIVE_MSG_2_CALLBACK */

        TX_CAN_RX[2u].rxcmd.byte[0u] |= TX_CAN_RX_ACK_MSG;
    }
    ...
    /*******************************************************************************
    * FUNCTION NAME:   TX_CAN_ReceiveMsg3
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    void TX_CAN_ReceiveMsg3(void) 
    {
        ...
        #ifdef TX_CAN_RECEIVE_MSG_3_CALLBACK
            TX_CAN_ReceiveMsg_3_Callback();
        #endif /* TX_CAN_RECEIVE_MSG_3_CALLBACK */

        TX_CAN_RX[3u].rxcmd.byte[0u] |= TX_CAN_RX_ACK_MSG;
    }
    ...
    /*******************************************************************************
    * FUNCTION NAME:   TX_CAN_ReceiveMsg4
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    void TX_CAN_ReceiveMsg4(void) 
    {
        ...
        #ifdef TX_CAN_RECEIVE_MSG_4_CALLBACK
            TX_CAN_ReceiveMsg_4_Callback();
        #endif /* TX_CAN_RECEIVE_MSG_4_CALLBACK */

        TX_CAN_RX[4u].rxcmd.byte[0u] |= TX_CAN_RX_ACK_MSG;
    }
    ...
    /*******************************************************************************
    * FUNCTION NAME:   TX_CAN_ReceiveMsg5
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    void TX_CAN_ReceiveMsg5(void) 
    {
        ...
        #ifdef TX_CAN_RECEIVE_MSG_5_CALLBACK
            TX_CAN_ReceiveMsg_5_Callback();
        #endif /* TX_CAN_RECEIVE_MSG_5_CALLBACK */

        TX_CAN_RX[5u].rxcmd.byte[0u] |= TX_CAN_RX_ACK_MSG;
    }
    ...
    /*******************************************************************************
    * FUNCTION NAME:   TX_CAN_ReceiveMsg6
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    void TX_CAN_ReceiveMsg6(void) 
    {
        ...
        #ifdef TX_CAN_RECEIVE_MSG_6_CALLBACK
            TX_CAN_ReceiveMsg_6_Callback();
        #endif /* TX_CAN_RECEIVE_MSG_6_CALLBACK */

        TX_CAN_RX[6u].rxcmd.byte[0u] |= TX_CAN_RX_ACK_MSG;
    }
    ...
    /*******************************************************************************
    * FUNCTION NAME:   TX_CAN_ReceiveMsg7
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    void TX_CAN_ReceiveMsg7(void) 
    {
        ...
        #ifdef TX_CAN_RECEIVE_MSG_7_CALLBACK
            TX_CAN_ReceiveMsg_7_Callback();
        #endif /* TX_CAN_RECEIVE_MSG_7_CALLBACK */

        TX_CAN_RX[7u].rxcmd.byte[0u] |= TX_CAN_RX_ACK_MSG;
    }
    ...
    /*******************************************************************************
    * FUNCTION NAME:   TX_CAN_ReceiveMsg8
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    void TX_CAN_ReceiveMsg8(void) 
    {
        ...
        #ifdef TX_CAN_RECEIVE_MSG_8_CALLBACK
            TX_CAN_ReceiveMsg_8_Callback();
        #endif /* TX_CAN_RECEIVE_MSG_8_CALLBACK */

        TX_CAN_RX[8u].rxcmd.byte[0u] |= TX_CAN_RX_ACK_MSG;
    }
    ...
    /*******************************************************************************
    * FUNCTION NAME:   TX_CAN_ReceiveMsg9
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    void TX_CAN_ReceiveMsg9(void) 
    {
        ...
        #ifdef TX_CAN_RECEIVE_MSG_9_CALLBACK
            TX_CAN_ReceiveMsg_9_Callback();
        #endif /* TX_CAN_RECEIVE_MSG_9_CALLBACK */

        TX_CAN_RX[9u].rxcmd.byte[0u] |= TX_CAN_RX_ACK_MSG;
    }
    ...
    /*******************************************************************************
    * FUNCTION NAME:   TX_CAN_ReceiveMsg10
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    void TX_CAN_ReceiveMsg10(void) 
    {
        ...
        #ifdef TX_CAN_RECEIVE_MSG_10_CALLBACK
            TX_CAN_ReceiveMsg_10_Callback();
        #endif /* TX_CAN_RECEIVE_MSG_10_CALLBACK */

        TX_CAN_RX[10u].rxcmd.byte[0u] |= TX_CAN_RX_ACK_MSG;
    }
    ...
    /*******************************************************************************
    * FUNCTION NAME:   TX_CAN_ReceiveMsg11
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    void TX_CAN_ReceiveMsg11(void) 
    {
        ...
        #ifdef TX_CAN_RECEIVE_MSG_11_CALLBACK
            TX_CAN_ReceiveMsg_11_Callback();
        #endif /* TX_CAN_RECEIVE_MSG_11_CALLBACK */

        TX_CAN_RX[11u].rxcmd.byte[0u] |= TX_CAN_RX_ACK_MSG;
    }
    ...
    /*******************************************************************************
    * FUNCTION NAME:   TX_CAN_ReceiveMsg12
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    void TX_CAN_ReceiveMsg12(void) 
    {
        ...
        #ifdef TX_CAN_RECEIVE_MSG_12_CALLBACK
            TX_CAN_ReceiveMsg_12_Callback();
        #endif /* TX_CAN_RECEIVE_MSG_12_CALLBACK */

        TX_CAN_RX[12u].rxcmd.byte[0u] |= TX_CAN_RX_ACK_MSG;
    }
    ...
    /*******************************************************************************
    * FUNCTION NAME:   TX_CAN_ReceiveMsg13
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    void TX_CAN_ReceiveMsg13(void) 
    {
        ...
        #ifdef TX_CAN_RECEIVE_MSG_13_CALLBACK
            TX_CAN_ReceiveMsg_13_Callback();
        #endif /* TX_CAN_RECEIVE_MSG_13_CALLBACK */

        TX_CAN_RX[13u].rxcmd.byte[0u] |= TX_CAN_RX_ACK_MSG;
    }
    ...
    /*******************************************************************************
    * FUNCTION NAME:   TX_CAN_ReceiveMsg14
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    void TX_CAN_ReceiveMsg14(void) 
    {
        ...
        #ifdef TX_CAN_RECEIVE_MSG_14_CALLBACK
            TX_CAN_ReceiveMsg_14_Callback();
        #endif /* TX_CAN_RECEIVE_MSG_14_CALLBACK */

        TX_CAN_RX[14u].rxcmd.byte[0u] |= TX_CAN_RX_ACK_MSG;
    }
    ...
    /*******************************************************************************
    * FUNCTION NAME:   TX_CAN_ReceiveMsg15
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    void TX_CAN_ReceiveMsg15(void) 
    {
        ...
        #ifdef TX_CAN_RECEIVE_MSG_15_CALLBACK
            TX_CAN_ReceiveMsg_15_Callback();
        #endif /* TX_CAN_RECEIVE_MSG_15_CALLBACK */

        TX_CAN_RX[15u].rxcmd.byte[0u] |= TX_CAN_RX_ACK_MSG;
    }
