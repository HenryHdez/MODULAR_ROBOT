...
void Evaluar_Fitness(){
    CAN_isr_Stop();
    int i=0;
    for(i=0;i<Individuos;i++){
        Enviar_Parametros(Poblacion[0][i],Poblacion[1][i],Poblacion[2][i],V);
        CyDelay(50);
        Enviar_Parametros(Poblacion[0][i],Poblacion[1][i],Poblacion[2][i],V);
        CyDelay(30000);
        Fitness[i]=Medir_dist();
    }
...
void Cruce(){
    ...
    float New_Componente;
    New_Componente=Poblacion[componente][papa];
    Poblacion[componente][papa]=Poblacion[componente][mama];
    Poblacion[componente][mama]=New_Componente;
}

void Mutacion(){
    int Selector=(int)Individuos*randd();
    int componente=(int)randd()*2;
    if(componente==0){Poblacion[componente][Selector]=random_amplitud();}
    else if(componente==1){Poblacion[componente][Selector]=random_Fase();}
    else if(componente==2){Poblacion[componente][Selector]=random_Offset();}
}
...
int main()
{
    ...
    for(i=0;i<10;i++){
        for(j=0;j<3;j++){
            if(j==0){Poblacion[j][i]=random_amplitud();}
            else if(j==1){Poblacion[j][i]=random_Fase();}
            else if(j==2){Poblacion[j][i]=random_Offset();}
        }
    }
    //Init LED's
    CAN_Start(); ///  start CAN
